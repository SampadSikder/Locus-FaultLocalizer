...
//			System.out.println(lines.get(index));
		...
		try {
			for (String line : lines) {
				...
				if (date.getTime()/1000 < startTime)
		...
		for (Bug bug : bugs) {
			...
			for (int sid : sourceCommits.keySet()) {
				...
				for (String commit : commits) {
					if (time > commitTime.get(commit)/1000) {
						if (!isCommitFix.containsKey(commit))
							continue;
						if (isCommitFix.get(commit)) {
							System.out.println("calculating sus");
							double norm = (commitTime.get(commit)/1000 - startTime) * 1.0 / (time - startTime);
